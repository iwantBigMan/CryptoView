...
CryptoView는 Google 계정으로 로그인한 뒤 여러 거래소의 보유 자산을 한 화면에서 확인하는 Android 앱입니다. 현재는 Upbit와 Gate.io 중심으로 자산 조회, 거래소별 요약, 보유 코인 목록, 코인별 상세 정보를 제공합니다.

Upbit API Key/Secret은 앱 기기에 저장하지 않습니다. 앱은 Firebase ID Token으로 백엔드에 인증 요청을 보내고, Upbit 키 검증과 저장 및 삭제는 백엔드에서 처리합니다. 기기에는 Upbit 연동 완료 여부를 나타내는 `UPBIT_LINKED` marker만 저장합니다.
...
- 로그아웃 시 Google/Firebase 세션 종료, 로컬 캐시 초기화, 백엔드 Upbit credential 삭제 시도
...
- Upbit
  ...
  - 기기에는 API Key/Secret을 저장하지 않고 연동 marker만 저장
- Gate.io
  - Spot/Futures API 모델과 repository 구조 보유
  - 현재 일부 로컬 credential 기반 코드가 남아 있음
  - 백엔드 구현 후 재작업 예정
- Binance, Bybit
...
- 특정 코인의 거래소별 보유 수량, 평균 매수가, 현재가, 평가 금액, 손익 표시
- KRW/USDT 가격 단위 구분
...
- Gate.io 관련 DTO 의존성 일부는 백엔드 재작업 예정 범위로 남아 있습니다.
...
- OkHttp Interceptor: Firebase 토큰 및 Gate.io HMAC 인증 주입
...
| `domain.model.exchange` | 거래소 타입 및 거래소 credential 모델 |
| `domain.model.auth` | 인증 및 credential 처리 결과 모델 |
...
- Upbit API Key/Secret은 기기에 저장하지 않습니다.
- Upbit API Key/Secret은 백엔드에서 Google KMS 기반으로 관리합니다.
- 기기에는 Upbit 연동 여부 marker인 `UPBIT_LINKED`만 저장합니다.
- Gate.io 등 기타 거래소의 로컬 credential 코드는 백엔드 저장 방식으로 통합할 예정입니다.
...
KSP 버전을 Kotlin 2.3.0에 맞춰 올리거나 Kotlin 버전을 KSP에 맞춰 조정할 필요가 있습니다.
...
- Gate.io credential을 백엔드 저장 방식으로 통합
- Gate.io domain DTO 의존성 제거
